...
import torch
import time
sys.path.append('absolute path of timelens')
# Not set right, It will cause the error:ModuleNotFoundError: No module named 'timelens'
sys.path.append("..")
import torch as th
...
def _interpolate(
    ...
    for (left_frame, right_frame), event_sequence in combined_iterator:
        ...
        #The new timestamps about interpolation
        ...
        for split_index, (left_events, right_events) in enumerate(iterator_over_splits):
            ...
            example = pytorch_tools.train_move_tensors_to_cuda(device,example)
        ...
        print('average time:',np.mean(use_time))
        print("lately time:",np.mean(use_time[-10:]) )
...
if __name__ == "__main__":
    ...
    number_of_frames_to_insert=199#the number of frames to insert between two frames
    def mysort(key):
        return int(key)
    checkpoint_file = './refined_model/attention.bin'
    singale_src_dir='./tests/1'
    root_event_folder = os.path.join(singale_src_dir,'events')
    root_image_folder = os.path.join(singale_src_dir,'images')
    root_output_folder = os.path.join(singale_src_dir,'interpolated_frames')
    main(
        checkpoint_file,
        root_event_folder,
        root_image_folder,
