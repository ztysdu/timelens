...
class RefineWarp(warp_network.Warp, fusion_network.Fusion):
    ...
    def train_run_refine_warp(self, example,device):
        
        with torch.no_grad():
            warp_network.Warp.train_run_and_pack_to_example(self, example,device)
            fusion_network.Fusion.run_and_pack_to_example(self, example)
        ...
        (after_residual, before_residual) = th.chunk(residual, 2, dim=1)
        residual = th.cat([after_residual, before_residual], dim=0)
        refined, refined_invalid = warp.train_backwarp_2d(
            source=_pack_images_for_second_warping(example),
            y_displacement=residual[:, 0, ...],
            x_displacement=residual[:, 1, ...],
            device=device,
    ...
    def train2_run_refine_warp(self, example,device):
        warp_network.Warp.train_run_and_pack_to_example(self, example,device)
        fusion_network.Fusion.run_and_pack_to_example(self, example)
    ...
    def train_run_and_pack_to_example(self, example,device):
        _pack_output_to_example(example, self.train2_run_refine_warp(example,device))
