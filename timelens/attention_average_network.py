...
def train_pack_input_for_attention_computation(example,device):
    fusion = example["middle"]["fusion"]
    number_of_examples, _, height, width = fusion.size()
    return th.cat(
        [
            ...
            example["middle"]["before_refined_warped"],
            example["middle"]["fusion"],
            th.tensor(example["middle"]["weight"],device=device)
            .view(-1, 1, 1, 1)
            .expand(number_of_examples, 1, height, width)
            .type(fusion.type()),
        ],
...
class AttentionAverage(refine_warp_network.RefineWarp):
    ...
    def train_run_attention_averaging(self, example,device):
        
        with torch.no_grad():
            refine_warp_network.RefineWarp.train_run_and_pack_to_example(self, example,device)
        attention_scores = self.attention_network(
            train_pack_input_for_attention_computation(example,device)
        )
        attention = F.softmax(attention_scores, dim=1)
        average = _compute_weighted_average(
            attention,
    ...
    def train2_run_attention_averaging(self, example,device):
        refine_warp_network.RefineWarp.train_run_and_pack_to_example(self, example,device)
        attention_scores = self.attention_network(
            train_pack_input_for_attention_computation(example,device)
        )
